...
// Calculate commission for an auction (5%)
export const calculateCommission = async (auctionId) => {
  const auction = await Auction.findById(auctionId);
  if (!mongoose.Types.ObjectId.isValid(auctionId) || !auction) {
    throw new Error("Invalid Auction Id or auction not found.");
  }
  const commissionRate = 0.05;
  const commission = auction.finalBidAmount * commissionRate;
  return Math.round(commission);
};

// Auctioneer submits commission payment proof
