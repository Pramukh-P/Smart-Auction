// frontend/src/store/slices/superAdminSlice.js
...
const superAdminSlice = createSlice({
  ...
  initialState: {
    loading: false,
    monthlyRevenue: [],       // ADMIN COMMISSION by month (from /monthlyincome -> Commission)
    totalAuctioneers: [],
    totalBidders: [],
    paymentProofs: [],
    singlePaymentProof: {},
...
export const getMonthlyRevenue = () => async (dispatch) => {
  ...
  try {
    // Now returns COMMISSION data, via getMonthlyCommissions
    const response = await axios.get(
      "http://localhost:5000/api/v1/superadmin/monthlyincome",
      { withCredentials: true }
    );

    const raw = response.data.totalMonthlyRevenue;
    ...
    let payload = new Array(12).fill(0);
    if (Array.isArray(raw)) {
      payload = raw
        .slice(0, 12)
        .map(toNumber)
        .concat(
          new Array(Math.max(0, 12 - raw.length)).fill(0)
        )
        .slice(0, 12);
    } else if (raw && typeof raw === "object") {
      payload = MONTH_LABELS.map((m, idx) => {
        const key1 = String(idx + 1);
        return toNumber(raw[key1] ?? raw[m] ?? raw[idx] ?? 0);
      });
    } else {
      payload = new Array(12).fill(0);
    }

    dispatch(superAdminSlice.actions.successForMonthlyRevenue(payload));
  } catch (error) {
